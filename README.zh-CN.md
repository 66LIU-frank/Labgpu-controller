...
![Status](https://img.shields.io/badge/status-v0.1.4--alpha-orange)
...
Enter Server
  ...
  -> 本机 LabGPU gateway 接到选中的 CC Switch provider
  -> 当前 provider
```
...
- LabGPU 会在本机 gateway 内使用当前选中的 CC Switch Claude/Codex provider 配置，
  不把真实 provider key 复制到远程服务器
...
Claude/Codex Proxy Tunnel 可能会在本机 gateway 内读取当前选中的 CC Switch provider
key，但远程服务器仍然只拿到 `labgpu-session-*` token。LabGPU 不创建或保存 provider key。
...
```bash
git tag -a v0.1.4-alpha -m "LabGPU v0.1.4 alpha"
...
- LabGPU 在 summary/switching 里只读 provider name、current selection、proxy port
- 真实 provider key 保持在 CC Switch 或本机 provider 工具里；Claude/Codex Proxy Tunnel
  只会在本机 gateway 内使用选中的 CC Switch provider key
