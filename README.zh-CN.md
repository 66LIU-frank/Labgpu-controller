...
Enter Server
  ...
  -> Claude 走本机 CC Switch proxy；Codex 可由本机 gateway 直接接到选中的 CC Switch provider
  -> 当前 provider
```
...
- 当 CC Switch 的 Codex proxy route 不能提供可用 `base_url` 时，LabGPU 会在本机
  gateway 内使用当前选中的 CC Switch Codex provider 配置
...
Codex Proxy Tunnel 可能会在本机 gateway 内读取当前选中的 CC Switch Codex provider
key，但远程服务器仍然只拿到 `labgpu-session-*` token。LabGPU 不创建或保存 provider
key。
...
- 真实 provider key 保持在 CC Switch 或本机 provider 工具里；Codex Proxy Tunnel
  只会在本机 gateway 内使用选中的 CC Switch Codex key
